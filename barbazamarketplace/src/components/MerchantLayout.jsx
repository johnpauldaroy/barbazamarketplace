...
const MerchantLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-[#1E293B] antialiased pb-[calc(72px+env(safe-area-inset-bottom,0px))] xl:pb-0">
      {/* Mobile top bar - navigation itself lives in the bottom bar */}
      <div className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-slate-200 bg-white px-4 xl:hidden">
        <button type="button" onClick={() => navigate('/merchant')} className="flex min-w-0 items-center gap-2">
          <img src={brandLogoSrc} alt="e-KoopMart" className="h-8 w-8 shrink-0 object-contain" />
          <span className="truncate text-sm font-bold text-[#303030]">{storeName}</span>
        </button>
        <button
          type="button"
          aria-label="Logout"
          ...
          className="flex h-9 shrink-0 items-center gap-1.5 rounded-lg border border-slate-200 px-3 text-xs font-bold text-[#C15555] disabled:opacity-60"
        >
