                ...
                {isPayLater(order) ? (
                  ...
                  <div className="grid gap-6 p-5 md:grid-cols-[220px_1fr]">
                    ...
                    <div className="space-y-4">
                      ...
                      <label className="block text-sm font-semibold text-slate-700">Transaction reference<input value={proofForms[order.id]?.referenceNumber || ''} onChange={(event) => updateProofForm(order.id, 'referenceNumber', event.target.value)} className="mt-1.5 min-h-11 w-full rounded-lg border border-slate-200 px-3 text-base outline-none focus:border-[#2954C8]" placeholder="Enter the reference from your receipt" /></label>
                      <label className="block text-sm font-semibold text-slate-700">Payment proof image<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => updateProofForm(order.id, 'proof', event.target.files?.[0] || null)} className="mt-1.5 min-h-11 w-full rounded-lg border border-slate-200 bg-white p-2 text-sm" /></label>
